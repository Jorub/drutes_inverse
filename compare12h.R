...
plot(sim_12h, xlim = xlims, ylim = ylims, type = "l", col ="royalblue3", lwd = 2,
     xlab = "", ylab = "", xaxt = "n", yaxt = "n")
leg.txt <- c("Measured 0 h", "Measured 12 h",  "Sim 0 h", "Sim 12 h")
legend("bottomright", leg.txt, col = c("black", gray(0.3),  "royalblue4", "royalblue3"),
       lty = c(rep(0,2),rep(1,2)), pch = c(4,6,NA,NA), lwd = 2)
dev.off()